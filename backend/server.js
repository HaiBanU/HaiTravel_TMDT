...
// --- API ROUTES ---
// (Toàn bộ các route User, Tour, Cart giữ nguyên không đổi)
...
// [PHIÊN BẢN SỬA LỖI CUỐI CÙNG] Route để chuyển văn bản thành giọng nói
    ...
    try {
        const fptResponse = await fetch('https://api.fpt.ai/hmi/tts/v5', {
            ...
            headers: {
                ...
                'Content-Type': 'application/x-www-form-urlencoded' // Thêm lại header này
            },
            // Gửi dưới dạng form-data với giọng đọc "banmai"
        ...
        if (data && data.async) {
            ...
            res.status(500).json({ error: 'Không thể tạo file âm thanh từ FPT.AI.', details: data });
        }
    } catch (error) {
